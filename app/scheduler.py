...
async def send_task(bot: Bot, user_id: int, day_num:int|None=None):
    ...
    try:
        ...
        # Всегда отправляем изображение первым
        if os.path.exists(image_path):
            await bot.send_photo(user_id, FSInputFile(image_path))
        ...
        if len(formated_text) <= 1024:
            # Если текст короткий - отправляем аудио с текстом в подписи
            ...
            if os.path.exists(audio_path):
                await bot.send_audio(
                    chat_id=user_id,
                    audio=FSInputFile(audio_path),
                    title=f"Приветствую!" if i == 1 else f"День {i - 1}",
                    performer="Челлендж"
                )
            # Форматируем текст
