...
async def process_after_focus(callback: CallbackQuery, state: FSMContext) -> None:
    try:
        await state.update_data(after_focus=callback.data)
        new_focus = kb.get_text_by_id(callback.data, kb.focus_options, kb.focus_ids)
        ...
        if old_focus_text:
            await callback.message.edit_text(
                confirms.get('after_focus').format(old_focus_text, new_focus), reply_markup=None)
        else:
...
async def process_question_whats_changed(callback: CallbackQuery, state: FSMContext) -> None:
    try:
        ...
        await callback.message.answer(confirms.get('saved'))
        logging.info(
